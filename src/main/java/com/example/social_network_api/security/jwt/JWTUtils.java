...
public class JWTUtils {
    ...
    public String generateToken(UserDetails userDetails) {
        return Jwts.builder()
                // setSubject: gắn username vào phần payload của token
                ...
                // compact(): hoàn thiện và trả token dưới dạng String
                .compact();
    }
    ...
    public String extractUsername(String token) {
        return Jwts.parser()
                // setSigningKey: cung cấp key để giải mã token
                .setSigningKey(jwtSecret)

                // parseClaimsJws: giải mã token và xác thực chữ ký
                .parseClaimsJws(token)

                // getBody: lấy phần payload của token
                .getBody()

                // getSubject: lấy ra giá trị username
                .getSubject();
    }
    ...
    private boolean isTokenExpired(String token) {
        ...
        // Nếu thời gian hết hạn trước thời điểm hiện tại => đã hết hạn
